...
export default function InlineProviderSearch({ onAdded, onCancel, careRecipientLabel }: Props) {
    ...
    return (
      <div className="mt-2 space-y-2" onClick={(e) => e.stopPropagation()}>
        <button
          ...
          className="w-full flex items-center gap-2 rounded-lg border border-[#E5EAF2] bg-white px-3 py-2 text-xs font-medium text-[#071832] hover:bg-[#F8F9FA] transition text-left"
        >
          <LinkIcon size={12} className="text-[#1677FF] shrink-0" />
        ...
        <button
          ...
          className="w-full flex items-center gap-2 rounded-lg border border-[#E5EAF2] bg-white px-3 py-2 text-xs font-medium text-[#071832] hover:bg-[#F8F9FA] transition text-left"
        >
          <Search size={12} className="text-[#1677FF] shrink-0" />
          Search For New Provider
        </button>
        <button type="button" onClick={onCancel} className="text-[10px] text-[#4F5F73] hover:text-[#071832]">
          Cancel
        </button>
      </div>
    );
  ...
  if (mode === "link") {
    return (
      <div className="mt-2 space-y-2" onClick={(e) => e.stopPropagation()}>
        {loadingProviders ? (
          <p className="text-xs text-[#4F5F73]">Loading providers...</p>
        ) : existingProviders.length === 0 ? (
          <div>
            <p className="text-xs text-[#4F5F73]">No providers on file yet.</p>
            <button type="button" onClick={() => setMode("search")} className="mt-1 text-xs font-medium text-[#1677FF] underline underline-offset-2">
              Search for a new one
            </button>
          </div>
        ) : (
          <div className="max-h-48 overflow-y-auto divide-y divide-[#E5EAF2] rounded-lg border border-[#E5EAF2] bg-white">
            {existingProviders.map((p) => (
              ...
              >
                <div>
                  <div className="text-xs font-medium text-[#071832]">{p.name}</div>
                  {p.specialty && <div className="text-[10px] text-[#4F5F73]">{p.specialty}</div>}
                </div>
                <span className="text-[10px] font-medium text-[#1677FF]">Link</span>
              </button>
            ))}
          </div>
        )}
        <div className="flex gap-3">
          <button type="button" onClick={() => setMode("choose")} className="text-[10px] text-[#4F5F73] hover:text-[#071832]">
            ← Back
          </button>
          <button type="button" onClick={onCancel} className="text-[10px] text-[#4F5F73] hover:text-[#071832]">
            Cancel
  ...
  return (
    <div className="mt-2 space-y-2" onClick={(e) => e.stopPropagation()}>
      <div className="relative">
        <Search size={12} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[#4F5F73]" />
        <input
          ref={inputRef}
          type="text"
          value={query}
          ...
          className="w-full rounded-lg border border-[#E5EAF2] bg-white py-1.5 pl-8 pr-3 text-xs focus:outline-none focus:ring-1 focus:ring-[#1677FF]"
        />
        {searching && (
          <span className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[10px] text-[#4F5F73]">Searching...</span>
        )}
      </div>
          ...
          {results.slice(0, 5).map((result) => (
            <div key={result.npi || result.name} className="flex items-center justify-between gap-2 px-3 py-2">
              <div className="min-w-0 flex-1">
                <div className="text-xs font-medium text-[#071832] truncate">{result.name}</div>
      ...
      {query.length >= 2 && !searching && results.length === 0 && (
        <div className="py-2 text-center text-[10px] text-[#4F5F73]">No results found</div>
      )}
