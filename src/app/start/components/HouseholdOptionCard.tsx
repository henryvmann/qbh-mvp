...
export function HouseholdOptionCard({ title, description, selected, onClick }: Props) {
  return (
    <button
      ...
      onClick={onClick}
      className={[
        "w-full text-left rounded-2xl bg-[#131B2E] p-5 ring-1 transition",
        selected ? "ring-[#D4A843] ring-2" : "ring-white/8 hover:ring-white/15",
      ].join(" ")}
    >
      <div className="flex items-start justify-between gap-4">
        ...
        <div
          className={[
            ...
            selected ? "bg-[#D4A843] ring-[#D4A843]" : "bg-[#162030] ring-white/15",
          ].join(" ")}
          aria-hidden
        >
          {selected ? <div className="h-2 w-2 rounded-full bg-[#0B1120]" /> : null}
        </div>
      </div>
    </button>
  );
