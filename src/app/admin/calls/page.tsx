...
function ScoreBadge({ score }: { score: number }) {
  const color = score >= 4 ? "#1677FF" : score >= 3 ? "#B8A020" : "#C03020";
  return (
    <span
      ...
      <div className="mx-auto max-w-4xl px-6 pt-8 pb-20">
        ...
        {calls.some((c) => c.scorecard) && (
          <div className="mt-6 grid grid-cols-3 gap-3">
            <div className="rounded-xl bg-white border border-[#E5EAF2] shadow-sm p-4 text-center">
              <div className="text-2xl font-light text-[#1677FF]">
                {(calls.filter((c) => c.scorecard).reduce((sum, c) => sum + (c.scorecard?.overall_score || 0), 0) / calls.filter((c) => c.scorecard).length).toFixed(1)}
              </div>
              <div className="text-xs text-[#4F5F73]">Avg Score</div>
            </div>
            <div className="rounded-xl bg-white border border-[#E5EAF2] shadow-sm p-4 text-center">
              <div className="text-2xl font-light text-[#1677FF]">
                {calls.filter((c) => c.scorecard?.booked).length}
              </div>
              <div className="text-xs text-[#4F5F73]">Booked</div>
            </div>
            <div className="rounded-xl bg-white border border-[#E5EAF2] shadow-sm p-4 text-center">
              <div className="text-2xl font-light text-[#C03020]">
                {calls.filter((c) => c.scorecard && (c.scorecard.issues?.length || 0) > 0).length}
              </div>
              <div className="text-xs text-[#4F5F73]">With Issues</div>
            </div>
          </div>
        )}
          ...
          <div className="mt-6 rounded-2xl bg-white border border-[#E5EAF2] shadow-sm p-5">
            ...
            <div className="space-y-2">
              {Array.from(new Set(
                ...
                <div key={i} className="flex gap-2 text-xs text-[#4F5F73]">
        ...
        <div className="mt-6 space-y-3">
          {calls.map((call) => (
            <div
              key={call.attempt_id}
              className="rounded-2xl bg-white border border-[#E5EAF2] shadow-sm overflow-hidden"
            >
              <div
              ...
              >
                <div className="flex items-center gap-3">
                  {call.scorecard ? (
                    ...
                    <span className="h-8 w-8 rounded-full bg-[#E5EAF2]" />
                  )}
                  <div>
                    <div className="text-sm font-medium text-[#071832]">
                      Attempt #{call.attempt_id}
                      {call.scorecard?.booked && (
                        <span className="ml-2 text-xs text-[#1677FF]">✓ Booked</span>
                      )}
                    </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {!call.scorecard && (
                    <button
                      ...
                      className="rounded-lg px-3 py-1 text-xs font-medium text-[#1677FF] bg-[#1677FF]/10 hover:bg-[#1677FF]/20 disabled:opacity-50"
                    >
              ...
              {expanded === call.attempt_id && call.scorecard && (
                <div className="border-t border-[#E5EAF2] p-4 bg-[#F0F2F5]/50">
                  {/* Score breakdown */}
                  <div className="grid grid-cols-3 gap-2 mb-4">
                    {Object.entries(call.scorecard.scores).map(([key, val]) => (
                      <div key={key} className="text-center">
                        <div className="text-lg font-light" style={{ color: val >= 4 ? "#1677FF" : val >= 3 ? "#B8A020" : "#C03020" }}>
                          {val}
                        </div>
                        <div className="text-[10px] text-[#4F5F73] capitalize">
                          {key.replace(/_/g, " ")}
                        </div>
                      </div>
                    ))}
                  ...
                  {call.scorecard.issues.length > 0 && (
                    <div className="mb-3">
                      ...
                      {call.scorecard.issues.map((issue, i) => (
                        <div key={i} className="text-xs text-[#4F5F73] flex gap-1">
                  ...
                  {call.scorecard.prompt_suggestions.length > 0 && (
                    <div>
                      <div className="text-xs font-semibold text-[#1677FF] mb-1">Prompt Suggestions</div>
                      {call.scorecard.prompt_suggestions.map((s, i) => (
                        <div key={i} className="text-xs text-[#4F5F73] flex gap-1">
                          <span className="text-[#1677FF]">→</span> {s}
                        </div>
                      ))}
                    </div>
                  )}
