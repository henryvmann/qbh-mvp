...
function SurveyAnswers({ answers }: { answers: any }) {
  if (!answers || typeof answers !== "object") {
    return <span style={{ color: "#4F5F73" }}>No survey data</span>;
  }
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
      {entries.map(([key, value]) => (
        <div key={key} style={{ fontSize: 13 }}>
          <span style={{ color: "#4F5F73" }}>{key}:</span>{" "}
          <span style={{ color: "#071832" }}>
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            <span style={{ color: "#071832", fontWeight: 600, fontSize: 15 }}>
              {user.name || "Unknown"}
            </span>
            <span style={{ color: "#4F5F73", fontSize: 13 }}>
              {user.email || "no email"}
            </span>
          </div>
          <div
          ...
          >
            <span style={{ color: "#4F5F73", fontSize: 12 }}>
              Signed up {formatDate(user.created_at)}
            </span>
            <span style={{ color: "#1677FF", fontSize: 13, fontWeight: 600 }}>
        ...
        <div style={{ color: "#4F5F73", fontSize: 18 }}>
          {expanded ? "\u25B2" : "\u25BC"}
        </div>
      </div>
      ...
      {expanded && (
        ...
        >
          <div>
            <div style={{ color: "#4F5F73", fontSize: 12, marginBottom: 4, textTransform: "uppercase", letterSpacing: 0.5 }}>
              Provider Breakdown
            </div>
            <div style={{ color: "#071832", fontSize: 14, lineHeight: 1.6 }}>
          ...
          <div style={{ gridColumn: "1 / -1" }}>
            <div style={{ color: "#4F5F73", fontSize: 11 }}>
      ...
      <div style={{ maxWidth: 1000, margin: "0 auto" }}>
        ...
        {loading && (
          <div style={{ textAlign: "center", padding: 60 }}>
            ...
            />
            <div style={{ color: "#4F5F73" }}>Loading user data...</div>
          ...
          <>
            ...
            >
              {[
                ...
                >
                  <div style={{ color: "#1677FF", fontSize: 28, fontWeight: 700 }}>
                    {stat.value}
                  </div>
                  <div style={{ color: "#4F5F73", fontSize: 13, marginTop: 4 }}>
                    {stat.label}
                  </div>
                </div>
              ))}
            ...
            {/* User list */}
            {users.length === 0 ? (
              <div style={{ textAlign: "center", color: "#4F5F73", padding: 40 }}>
                No users found.
              </div>
            ) : (
